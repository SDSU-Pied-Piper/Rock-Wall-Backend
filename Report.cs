    ...
    class Report
    {
        ...
        private void suspensionsSetUp(XLWorkbook workbook, List<IXLWorksheet> workSheets)
        {
            ...
            for (int i = 0; i < suspensions.Tables[0].Rows.Count; i++)
            {
                ...
                {
                    workSheets[2].Cell(cell.ToString()).Value = suspensions.Tables[0].Rows[i][j];
                    cell[0] = (char)((int)(cell[0]) + 1);
                }
                cell[0] = 'A';
                cell[1] = (char)((int)(cell[1]) + 1);
        ...
        {
            Database db = new Database();
            SqlCommand command = new SqlCommand("Select firstName,lastName,studentID,dateOfBirth,gender,isBlayCertified,isLeadCertified,lastCheckIn,numberofCheckIns,mailingList from Patrons where isWaiverSigned=1", db.con);
            DataSet suspensions = db.sendSelectCommand(command);
            StringBuilder cell = new StringBuilder("A2");
            for (int i = 0; i < suspensions.Tables[0].Rows.Count; i++)
            {
